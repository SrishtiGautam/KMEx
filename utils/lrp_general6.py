...
from utils.dataloader_nrm import *
...
class conv2d_zbeta_wrapper_class(nn.Module):
  def __init__(self, module, lrpignorebias,lowest  = None, highest = None, dataset=None):
    super(conv2d_zbeta_wrapper_class, self).__init__()

    if lowest is None:
      # lowest=torch.min(torch.tensor([-0.485/0.229, -0.456/0.224, -0.406/0.225]))
      # lowest = torch.min(torch.tensor([-0.5 / 0.5, -0.5 / 0.5, -0.5 / 0.5]))
        lowest = torch.min(-torch.tensor(NRM[dataset][0])/torch.tensor(NRM[dataset][1]))
    if highest is None:
      # highest=torch.max (torch.tensor([(1-0.485)/0.229, (1-0.456)/0.224, (1-0.406)/0.225]))
      # highest = torch.max(torch.tensor([(1 - 0.5) / 0.5, (1 - 0.5) / 0.5, (1 - 0.5) / 0.5]))
        highest = torch.max((1-torch.tensor(NRM[dataset][0]))/torch.tensor(NRM[dataset][1]))
...
def get_lrpwrapperformodule(module, lrp_params, lrp_layer2method, thisis_inputconv_andiwant_zbeta=False, dataset=None):
  ...
  if isinstance(module, nn.ReLU):
    ...
    if True== thisis_inputconv_andiwant_zbeta:
      return conv2d_zbeta_wrapper_class(module , lrp_params['conv2d_ignorebias'], dataset=dataset)
    else:
      key='nn.Conv2d'
      if key not in lrp_layer2method:
        print("found no dictionary entry in lrp_layer2method for this module name:", key)
...
class identity_wrapper_fct(torch.autograd.Function): # to be used with generic_activation_pool_wrapper_class(module,this)
    """
    We can implement our own custom autograd Functions by subclassing
    torch.autograd.Function and implementing the forward and backward passes
    which operate on Tensors.
...
class l2_lrp(torch.autograd.Function):
    ...
    def forward(ctx, conv_features, model, pno):
        ...
        ctx.conv_features = conv_features
        ctx.prototype_vectors = model.prototype_vectors
        ctx.pno = pno
    ...
    def backward(ctx, grad_output):
        ...
        # print("l2 custom backward")
        # conv, prototypes, pno = ctx.saved_tensors
        conv = ctx.conv_features
        prototypes = ctx.prototype_vectors
        pno = ctx.pno
        pno = pno.item()
        i = conv.shape[2]
        j = conv.shape[3]
...
class sim(torch.autograd.Function):
    @staticmethod
    def forward(ctx, ebd, prt):
        dist = (ebd - prt)**2
        similarities = torch.log((dist + 1) / (dist + 1e-8))
...
class l2_lrp_class(torch.autograd.Function):
    @staticmethod
    def forward(ctx, conv_features, model, prt):
        prt = torch.unsqueeze(torch.unsqueeze(prt,dim=2),dim=3)
        # pno = torch.tensor([pno], dtype=torch.int32)
        ctx.save_for_backward(conv_features, prt)  # *values unpacks the list
        # print('l2 custom forward')
        x2 = conv_features ** 2
        x2_patch_sum = F.conv2d(input=x2, weight=model.ones)
        ...
        xp = F.conv2d(input=conv_features, weight=prt)
        intermediate_result = - 2 * xp + p2_reshape  # use broadcast
        # x2_patch_sum and intermediate_result are of the same shape
        distances = F.relu(x2_patch_sum + intermediate_result)
    ...
    def backward(ctx, grad_output):
        ...
        # conv = conv.squeeze()     ##Questionable line

        l2 = (conv - prototype) ** 2
        d = 1 / (l2 ** 2 + 1e-12)  ##time consuming

        denom = torch.sum(d, dim=1, keepdim=True) + 1e-12   ##crazy time consuming
        denom = denom.repeat(1, c, 1, 1) + 1e-12    ##time consuming
        R = torch.div(d, denom)     ##crazy time consuming

        # grad_output = grad_output[:,pno,:,:].squeeze()
        grad_output = grad_output.repeat(c, 1, 1, 1)    ##crazy time consuming
